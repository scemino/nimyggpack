const bnutKey = [
  0x04'u8, 0x1f, 0x5a, 0xac, 0x5f, 0x79, 0x10, 0xaf, 0x04, 0x1d, 0x46, 0x3a,
...
proc bnutDecode*(bytes: string): string =
  var cursor = bytes.len and 255
  for b in bytes:
    let value = b.uint8 xor bnutKey[cursor]
    cursor = (cursor + 1) mod bnutKey.len
    result.add(value.char)

proc bnutEncode*(text: string): string =
  bnutDecode(text)
